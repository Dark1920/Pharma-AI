...
.stTabs [aria-selected="true"] {
    ...
    border-bottom-color: #1a8a54 !important;
...
with tab1:
    ...
    with col_l:
        ...
        custom = st.text_input(
            "", placeholder="e.g. What compounds in Aloe vera help wound healing?",
...
<div class="mstrip">
  <div class="mcard">
    <div class="mlabel">CPU time</div>
    <div class="mval">{cpu_total:.2f}s</div>
  </div>
  <div class="mcard">
    <div class="mlabel">AMD GPU estimate</div>
    <div class="mval">{gpu_total:.2f}s</div>
    <div class="msub">~10× faster</div>
  </div>
  <div class="mcard">
    <div class="mlabel">Sources</div>
    <div class="mval">{len(sources)}</div>
  </div>
</div>""", unsafe_allow_html=True)
...
<div class="src">
  ...
  <div class="src-text">{doc.page_content[:220]}...</div>
</div>""", unsafe_allow_html=True)
...
<div style="height:260px;display:flex;flex-direction:column;align-items:center;
            justify-content:center;color:#bbb;gap:10px;
            border:1.5px dashed #e8e8e8;border-radius:12px;">
    ...
    try:
        ...
        def risk_card(r, mode="drug"):
            ...
            if mode == "drug":
                ...
                detail = (f"<b>Compound:</b> {r['compound']}<br>"
                          f"<b>Effect:</b> {r['effect']}<br>"
                          f"<b>Mechanism:</b> {r['mechanism']}<br>"
                          f"<b>⚕️ Action:</b> {r['action']}")
            else:
                title  = f"{icon} {risk} · {r['plant_1']} × {r['plant_2']}"
                detail = (f"<b>Compounds:</b> {r['compounds']}<br>"
                          f"<b>Effect:</b> {r['effect']}<br>"
                          f"<b>Mechanism:</b> {r['mechanism']}<br>"
                          f"<b>⚕️ Recommendation:</b> {r['action']}")
        ...
        if stype == "Plant + Drug":
            ...
            if go:
                ...
                if res:
                    ...
                    st.markdown("""<div style="padding:1rem;background:#fafafa;border:1px solid #efefef;
                    border-radius:10px;font-size:.87rem;color:#666;">
                    No known interaction found — always consult a healthcare professional.</div>""", unsafe_allow_html=True)

            st.markdown("<hr>", unsafe_allow_html=True)
            st.markdown('<div class="slabel">Quick examples</div>', unsafe_allow_html=True)
            e1, e2, e3, e4 = st.columns(4)
            examples = [("🔴 Moringa + Warfarin","Moringa","Warfarin"),
                        ("🟡 Neem + Insulin","Neem","Insulin"),
                        ("🔴 Aloe + Digoxin","Aloe","Digoxin"),
                        ("🟡 Turmeric + Aspirin","Turmeric","Aspirin")]
            for col, (label, p, d) in zip([e1, e2, e3, e4], examples):
                with col:
                    if st.button(label, use_container_width=True):
            ...
            pp_examples = [("🟢 Turmeric + Ginger","Turmeric","Ginger"),
                           ("🟡 Moringa + Kinkeliba","Moringa","Kinkeliba"),
                           ("🟢 Moringa + Turmeric","Moringa","Turmeric")]
            for col, (label, a, b) in zip([e1, e2, e3], pp_examples):
                with col:
                    if st.button(label, use_container_width=True):
        ...
        st.warning("interactions_db.py not found. Add it to your project folder.")
...
<div class="mstrip">
  <div class="mcard">
    <div class="mlabel">CPU total</div>
    <div class="mval">{cpu_t:.2f}s</div>
  </div>
  <div class="mcard">
    <div class="mlabel">AMD GPU total</div>
    <div class="mval">{gpu_t:.2f}s</div>
    <div class="msub">MI300X projection</div>
  </div>
  <div class="mcard">
    <div class="mlabel">Speedup</div>
    <div class="mval">{spd:.1f}×</div>
  </div>
  <div class="mcard">
    <div class="mlabel">Time saved</div>
    <div class="mval">{saved:.2f}s</div>
  </div>
</div>""", unsafe_allow_html=True)
...
with tab4:
    ...
    with col_a:
        ...
        try:
            ...
            st.warning("composes_moringa.csv not found. Run pubchem_data.py first.")
    ...
    try:
        ...
        st.warning("moringa_articles.csv not found. Run pubmed_data.py first.")